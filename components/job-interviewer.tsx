    ...
    <div className="mx-auto max-w-6xl">
      ...
      <section className="mt-6 grid overflow-hidden rounded-4xl border-2 border-ink bg-white shadow-pop lg:grid-cols-[.75fr_1.25fr]">
        ...
        <form onSubmit={interview} className="p-5 md:p-9"><div className="grid gap-4 sm:grid-cols-2"><label className="text-xs font-black uppercase tracking-wider text-plum">Company<input value={company} onChange={(e) => setCompany(e.target.value)} placeholder="Acme Studio" className="mt-2 w-full rounded-2xl border-2 border-ink/15 px-4 py-3 text-base font-bold normal-case tracking-normal outline-none focus:border-coral"/></label><label className="text-xs font-black uppercase tracking-wider text-plum">Your title<input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Product Designer" className="mt-2 w-full rounded-2xl border-2 border-ink/15 px-4 py-3 text-base font-bold normal-case tracking-normal outline-none focus:border-coral"/></label></div>
          <div className="mt-5 flex items-center justify-between gap-3"><label htmlFor="story" className="text-xs font-black uppercase tracking-wider text-plum">What happened there?</label><select value={provider} onChange={(e) => setProvider(e.target.value as Provider)} aria-label="AI provider" className="rounded-xl border border-ink/15 bg-cream px-3 py-2 text-xs font-bold"><option value="openai">OpenAI</option><option value="anthropic">Claude</option><option value="google">Gemini</option><option value="ollama">Ollama</option></select></div>
          ...
          <button disabled={loading || !story.trim()} className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl border-2 border-ink bg-coral py-3 font-black shadow-pop transition disabled:cursor-not-allowed disabled:opacity-50">{loading ? <><LoaderCircle className="animate-spin" size={18}/> Listening…</> : <><Sparkles size={18}/> Find the strongest notes <Send size={17}/></>}</button>
