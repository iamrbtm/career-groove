...
function resolved(jobs: typeof validPayload.jobs) {
  return jobs.map((job) => ({ job, description: "test description", descriptionSource: "humanized" as const }));
}

function resolvedOne(job: typeof validPayload.jobs[number]) {
...
test("second import skips duplicates by dedupe_key", async () => {
  ...
  try {
    ...
    const first = await importJobEmailEntries(client, userId, resolved(validPayload.jobs), { searchRunDate: validPayload.search_run_date });
    const second = await importJobEmailEntries(client, userId, resolved(validPayload.jobs), { searchRunDate: validPayload.search_run_date });
...
test("skips duplicates by source_job_id for the same company", async () => {
  ...
  try {
    ...
    const first = resolvedOne({ ...job, canonical_url: null, dedupe_key: "a".repeat(64) });
    const second = resolvedOne({ ...first.job, dedupe_key: "b".repeat(64) });
...
test("classifies existing duplicates before quota checks", async () => {
  ...
  try {
    const userId = await seedUser(client);
    const [job] = validPayload.jobs;
    await importJobEmailEntries(client, userId, [resolvedOne(job)], { searchRunDate: validPayload.search_run_date });
...
test("skips duplicates by canonical_url", async () => {
  ...
  try {
    const userId = await seedUser(client);
    const [job] = validPayload.jobs;
    const first = resolvedOne({ ...job, source_job_id: null, dedupe_key: "c".repeat(64) });
    const second = resolvedOne({ ...first.job, company: "Another Company", dedupe_key: "d".repeat(64) });
    await importJobEmailEntries(client, userId, [first], { searchRunDate: validPayload.search_run_date });
    const result = await importJobEmailEntries(client, userId, [second], { searchRunDate: validPayload.search_run_date });
    assert.equal(result.duplicatesSkipped, 1);
  } finally {
...
test("stores JSON-derived salary and import metadata", async () => {
  ...
  try {
    const userId = await seedUser(client);
    await importJobEmailEntries(client, userId, resolved(validPayload.jobs), { searchRunDate: validPayload.search_run_date });
...
test("records the creation event and latest tracker score", async () => {
  ...
  try {
    const userId = await seedUser(client);
    const result = await importJobEmailEntries(client, userId, resolved(validPayload.jobs), { searchRunDate: validPayload.search_run_date });
...
test("concurrent imports with a shared fallback key create one application", async () => {
  ...
  try {
    ...
    const [job] = validPayload.jobs;
    const first = resolvedOne({ ...job, dedupe_key: "e".repeat(64) });
    const second = resolvedOne({ ...job, dedupe_key: "f".repeat(64) });
