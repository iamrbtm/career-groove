...
export const contactCreateSchema = z
  .object({
    ...
    email: z.preprocess(
      emptyToNull,
      z.string().trim().toLowerCase().email().max(320).nullable().default(null),
    ),
...
export const residenceCreateSchema = z
  ...
  .strict()
  .refine(
    ({ startedOn, endedOn }) => !startedOn || !endedOn || startedOn <= endedOn,
...
export const credentialCreateSchema = z
  ...
  .strict()
  .refine(
    ({ issuedOn, expiresOn }) => !issuedOn || !expiresOn || issuedOn <= expiresOn,
    { message: "expiresOn must not be before issuedOn", path: ["expiresOn"] },
  );
