...
const resumeSchema = z.object({
  ...
  education: z.array(z.object({
    ...
    issuer: z.string().nullable().describe("School or institution, or null when not supplied."),
    location: z.string().nullable().describe("School location, or null when not supplied."),
    date: z.string().nullable().describe("Concise graduation or attendance date copied from the source, or null when not supplied."),
    details: z.string().nullable().describe("Optional relevant detail of no more than 18 words, or null when not useful."),
  })).max(4),
  workExperience: z.array(z.object({
    company: z.string(),
    title: z.string(),
    location: z.string().nullable().describe("Work location, or null when not supplied."),
    startDate: z.string().nullable().describe("Concise month/year or year copied from the source, or null when not supplied."),
    endDate: z.string().nullable().describe("Concise month/year, year, Present, or null when not supplied."),
  ...
  certifications: z.array(z.object({
    name: z.string(),
    issuer: z.string().nullable().describe("Issuing organization, or null when not supplied."),
    date: z.string().nullable().describe("Issue or expiration date copied from the source when useful, or null when not supplied."),
    details: z.string().nullable().describe("Optional relevant detail of no more than 18 words, or null when not useful."),
  })).max(6),
